...
def _blur2d(x, f=[1,2,1], normalize=True, flip=False, stride=1):
    ...
    x = tf.nn.depthwise_conv2d(input=x, filter=f, strides=strides, padding='SAME', data_format='NCHW')
...
def _downscale2d(x, factor=2, gain=1):
    ...
    return tf.nn.avg_pool2d(input=x, ksize=ksize, strides=ksize, padding='VALID', data_format='NCHW')
...
def upscale2d_conv2d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    ...
    w = get_weight([kernel, kernel, x.shape[1].value, fmaps], **kwargs)
    w = tf.transpose(a=w, perm=[0, 1, 3, 2]) # [kernel, kernel, fmaps_out, fmaps_in]
...
def apply_bias(x, lrmul=1):
    b = tf.compat.v1.get_variable('bias', shape=[x.shape[1]], initializer=tf.compat.v1.initializers.zeros()) * lrmul
    b = tf.cast(b, x.dtype)
    if len(x.shape) == 2:
        return x + b
...
def leaky_relu(x, alpha=0.2):
    with tf.compat.v1.variable_scope('LeakyReLU'):
        ...
        def func(x):
            ...
            @tf.custom_gradient
            def grad(dy):
                dx = tf.compat.v1.where(y >= 0, dy, dy * alpha)
                return dx, lambda ddx: tf.compat.v1.where(y >= 0, ddx, ddx * alpha)
            return y, grad
        return func(x)
...
def instance_norm(x, epsilon=1e-8):
    ...
    with tf.compat.v1.variable_scope('InstanceNorm'):
        ...
        epsilon = tf.constant(epsilon, dtype=x.dtype, name='epsilon')
        x *= tf.math.rsqrt(tf.reduce_mean(input_tensor=tf.square(x), axis=[2,3], keepdims=True) + epsilon)
        x = tf.cast(x, orig_dtype)
        return x
...
def apply_noise(x, noise_var=None, randomize_noise=True):
    ...
    with tf.compat.v1.variable_scope('Noise'):
        ...
        weight = tf.compat.v1.get_variable('weight', shape=[x.shape[1].value], initializer=tf.compat.v1.initializers.zeros())
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    with tf.compat.v1.variable_scope('MinibatchStddev'):
        ...
        y -= tf.reduce_mean(input_tensor=y, axis=0, keepdims=True)           # [GMncHW] Subtract mean over group.
        y = tf.reduce_mean(input_tensor=tf.square(y), axis=0)                # [MncHW]  Calc variance over group.
        y = tf.sqrt(y + 1e-8)                                   # [MncHW]  Calc stddev over group.
        y = tf.reduce_mean(input_tensor=y, axis=[2,3,4], keepdims=True)      # [Mn111]  Take average over fmaps and pixels.
        y = tf.reduce_mean(input_tensor=y, axis=[2])                         # [Mn11] Split channels into c channel groups
...
def G_style(
    ...
    **kwargs):                                      # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    if style_mixing_prob is not None:
        with tf.compat.v1.name_scope('StyleMix'):
            ...
            mixing_cutoff = tf.cond(
                ...
                true_fn=lambda: tf.random.uniform([], 1, cur_layers, dtype=tf.int32),
                false_fn=lambda: cur_layers)
    ...
    if truncation_psi is not None and truncation_cutoff is not None:
        with tf.compat.v1.variable_scope('Truncation'):
            ...
            coefs = tf.compat.v1.where(layer_idx < truncation_cutoff, truncation_psi * ones, ones)
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.compat.v1.variable_scope('4x4'):
        if const_input_layer:
            with tf.compat.v1.variable_scope('Const'):
                x = tf.compat.v1.get_variable('const', shape=[1, nf(1), 4, 4], initializer=tf.compat.v1.initializers.ones())
                x = layer_epilogue(tf.tile(tf.cast(x, dtype), [tf.shape(input=dlatents_in)[0], 1, 1, 1]), 0)
        else:
    ...
    if structure == 'recursive':
        def cset(cur_lambda, new_cond, new_lambda):
            return lambda: tf.cond(pred=new_cond, true_fn=new_lambda, false_fn=cur_lambda)
        def grow(x, res, lod):
            y = block(res, x)
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    if label_size:
        with tf.compat.v1.variable_scope('LabelSwitch'):
            scores_out = tf.reduce_sum(input_tensor=scores_out * labels_in, axis=1, keepdims=True)
