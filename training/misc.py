...
def get_last_pkl_for_run_name(run_name):
    run_name = "-".join(run_name.split("-")[1:])
    for candidate in sorted(glob.glob("results/*-" + run_name))[::-1]:
        try:
            pkl = locate_network_pkl(candidate)
            return os.path.join(os.getcwd(), pkl)
            return pkl
        except:
            continue
    return None
