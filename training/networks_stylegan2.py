...
def dense_layer(x, fmaps, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    w = get_weight([x.shape.as_list()[1], fmaps], gain=gain, use_wscale=use_wscale, lrmul=lrmul, weight_var=weight_var)
...
def conv2d_layer(x, fmaps, kernel, up=False, down=False, resample_kernel=None, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    if up:
        x = upsample_conv_2d(x, tf.compat.v1.cast(w, x.dtype), data_format='NCHW', k=resample_kernel)
    elif down:
        x = conv_downsample_2d(x, tf.compat.v1.cast(w, x.dtype), data_format='NCHW', k=resample_kernel)
    else:
        x = tf.compat.v1.nn.conv2d(x, tf.compat.v1.cast(w, x.dtype), data_format='NCHW', strides=[1,1,1,1], padding='SAME')
    return x
...
def apply_bias_act(x, act='linear', alpha=None, gain=None, lrmul=1, bias_var='bias'):
    ...
    return fused_bias_act(x, b=tf.compat.v1.cast(b, x.dtype), act=act, alpha=alpha, gain=gain)
...
def naive_upsample_2d(x, factor=2):
    with tf.compat.v1.variable_scope('NaiveUpsample'):
        ...
        x = tf.compat.v1.reshape(x, [-1, C, H, 1, W, 1])
        x = tf.compat.v1.tile(x, [1, 1, 1, factor, 1, factor])
        return tf.compat.v1.reshape(x, [-1, C, H * factor, W * factor])

def naive_downsample_2d(x, factor=2):
    with tf.compat.v1.variable_scope('NaiveDownsample'):
        _N, C, H, W = x.shape.as_list()
        x = tf.compat.v1.reshape(x, [-1, C, H // factor, factor, W // factor, factor])
        return tf.math.reduce_mean(x, axis=[3,5])
...
def modulated_conv2d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, gain=1, use_wscale=True, lrmul=1, fused_modconv=True, weight_var='weight', mod_weight_var='mod_weight', mod_bias_var='mod_bias'):
    ...
    # Get weight.
    w = get_weight([kernel, kernel, x.shape.as_list()[1], fmaps], gain=gain, use_wscale=use_wscale, lrmul=lrmul, weight_var=weight_var)
    ww = w[np.newaxis] # [BkkIO] Introduce minibatch dimension.

    # Modulate.
    s = dense_layer(y, fmaps=x.shape.as_list()[1], weight_var=mod_weight_var) # [BI] Transform incoming W to style.
    s = apply_bias_act(s, bias_var=mod_bias_var) + 1 # [BI] Add bias (initially 1).
    ww *= tf.compat.v1.cast(s[:, np.newaxis, np.newaxis, :, np.newaxis], w.dtype) # [BkkIO] Scale input feature maps.

    # Demodulate.
    if demodulate:
        d = tf.math.rsqrt(tf.math.reduce_sum(tf.math.square(ww), axis=[1,2,3]) + 1e-8) # [BO] Scaling factor.
        ww *= d[:, np.newaxis, np.newaxis, np.newaxis, :] # [BkkIO] Scale output feature maps.

    # Reshape/scale input.
    if fused_modconv:
        x = tf.math.reshape(x, [1, -1, x.shape[2], x.shape[3]]) # Fused => reshape minibatch to convolution groups.
        w = tf.math.reshape(tf.transpose(ww, [1, 2, 3, 0, 4]), [ww.shape[1], ww.shape[2], ww.shape[3], -1])
    else:
        x *= tf.compat.v1.cast(s[:, :, np.newaxis, np.newaxis], x.dtype) # [BIhw] Not fused => scale input activations.
    ...
    if fused_modconv:
        x = tf.math.reshape(x, [-1, fmaps, x.shape[2], x.shape[3]]) # Fused => reshape convolution groups back to minibatch.
    elif demodulate:
        x *= tf.compat.v1.cast(d[:, :, np.newaxis, np.newaxis], x.dtype) # [BOhw] Not fused => scale output activations.
    return x
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    group_size = tf.math.minimum(group_size, tf.shape(x)[0])    # Minibatch must be divisible by (or smaller than) group_size.
    s = x.shape                                                 # [NCHW]  Input shape.
    y = tf.math.reshape(x, [group_size, -1, num_new_features, s[1]//num_new_features, s[2], s[3]])   # [GMncHW] Split minibatch into M groups of size G. Split channels into n channel groups c.
    y = tf.compat.v1.cast(y, tf.float32)                                  # [GMncHW] Cast to FP32.
    y -= tf.math.reduce_mean(y, axis=0, keepdims=True)                    # [GMncHW] Subtract mean over group.
    y = tf.math.reduce_mean(tf.square(y), axis=0)                         # [MncHW]  Calc variance over group.
    y = tf.math.sqrt(y + 1e-8)                                            # [MncHW]  Calc stddev over group.
    y = tf.math.reduce_mean(y, axis=[2,3,4], keepdims=True)               # [Mn111]  Take average over fmaps and pixels.
    y = tf.math.reduce_mean(y, axis=[2])                                  # [Mn11] Split channels into c channel groups
    y = tf.compat.v1.cast(y, x.dtype)                                     # [Mn11]  Cast back to original data type.
    y = tf.compat.v1.tile(y, [group_size, 1, s[2], s[3]])                 # [NnHW]  Replicate over group and pixels.
...
def G_main(
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    dlatents = tf.compat.v1.cast(dlatents, tf.float32)

    # Update moving average of W.
    if dlatent_avg_beta is not None:
        with tf.compat.v1.variable_scope('DlatentAvg'):
            batch_avg = tf.math.reduce_mean(dlatents[:, 0], axis=0)
            ...
            with tf.compat.v1.control_dependencies([update_op]):
                dlatents = tf.compat.v1.identity(dlatents)
    ...
    if style_mixing_prob is not None:
        with tf.compat.v1.variable_scope('StyleMix'):
            ...
            dlatents2 = tf.compat.v1.cast(dlatents2, tf.float32)
            layer_idx = np.arange(num_layers)[np.newaxis, :, np.newaxis]
            cur_layers = num_layers - tf.compat.v1.cast(lod_in, tf.int32) * 2
            mixing_cutoff = tf.cond(
                tf.compat.v1.random_uniform([], 0.0, 1.0) < style_mixing_prob,
                lambda: tf.compat.v1.random_uniform([], 1, cur_layers, dtype=tf.int32),
                lambda: cur_layers)
            dlatents = tf.compat.v1.where_v2(tf.compat.v1.broadcast_to(layer_idx < mixing_cutoff, tf.shape(dlatents)), dlatents, dlatents2)
    ...
    if truncation_psi is not None:
        with tf.compat.v1.variable_scope('Truncation'):
            ...
            if truncation_cutoff is None:
                ...
                layer_psi = tf.compat.v1.where_v2(layer_idx < truncation_cutoff, layer_psi * truncation_psi, layer_psi)
            dlatents = tflib.lerp(dlatent_avg, dlatents, layer_psi)

    # Evaluate synthesis network.
    deps = []
    if 'lod' in components.synthesis.vars:
        deps.append(tf.compat.v1.assign(components.synthesis.vars['lod'], lod_in))
    with tf.compat.v1.control_dependencies(deps):
...
def G_mapping(
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    latents_in = tf.compat.v1.cast(latents_in, dtype)
    labels_in = tf.compat.v1.cast(labels_in, dtype)
    x = latents_in

    # Embed labels and concatenate them with latents.
    if label_size:
        with tf.compat.v1.variable_scope('LabelConcat'):
            w = tf.compat.v1.get_variable('weight', shape=[label_size, latent_size], initializer=tf.keras.initializers.random_normal())
            y = tf.compat.v1.matmul(labels_in, tf.compat.v1.cast(w, dtype))
            x = tf.compat.v1.concat([x, y], axis=1)

    # Normalize latents.
    if normalize_latents:
        with tf.compat.v1.variable_scope('Normalize'):
            x *= tf.math.rsqrt(tf.math.reduce_mean(tf.math.square(x), axis=1, keepdims=True) + 1e-8)
    ...
    if dlatent_broadcast is not None:
        with tf.compat.v1.variable_scope('Broadcast'):
            x = tf.compat.v1.tile(x[:, np.newaxis], [1, dlatent_broadcast, 1])

    # Output.
    assert x.dtype == tf.compat.v1.as_dtype(dtype)
    return tf.compat.v1.identity(x, name='dlatents_out')
...
def G_synthesis_stylegan_revised(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    dlatents_in = tf.compat.v1.cast(dlatents_in, dtype)
    lod_in = tf.compat.v1.cast(tf.compat.v1.get_variable('lod', initializer=np.float32(0), trainable=False), dtype)
    ...
    for layer_idx in range(num_layers - 1):
        ...
        noise_inputs.append(tf.compat.v1.get_variable('noise%d' % layer_idx, shape=shape, initializer=tf.keras.initializers.RandomNormal(mean=0., stddev=1.), trainable=False))
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        if randomize_noise:
            ...
            noise = tf.compat.v1.cast(noise_inputs[layer_idx], x.dtype)
        noise_strength = tf.compat.v1.get_variable('noise_strength', shape=[], initializer=tf.keras.initializers.Zeros())
        x += noise * tf.compat.v1.cast(noise_strength, x.dtype)
        return apply_bias_act(x, act=act)

    # Early layers.
    with tf.compat.v1.variable_scope('4x4'):
        with tf.compat.v1.variable_scope('Const'):
            x = tf.compat.v1.get_variable('const', shape=[1, nf(1), 4, 4], initializer=tf.keras.initializers.RandomNormal(mean=0., stddev=1.))
            x = tf.compat.v1.tile(tf.compat.v1.cast(x, dtype), [tf.shape(dlatents_in)[0], 1, 1, 1])
    ...
    return tf.compat.v1.identity(images_out, name='images_out')
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.compat.v1.variable_scope('Output'):
        ...
        if labels_in.shape[1] > 0:
            x = tf.math.reduce_sum(x * labels_in, axis=1, keepdims=True)
    scores_out = x

    # Output.
    assert scores_out.dtype == tf.compat.v1.as_dtype(dtype)
    scores_out = tf.compat.v1.identity(scores_out, name='scores_out')
    return scores_out
...
def D_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    images_in.set_shape([None, num_channels, resolution, resolution])
    labels_in.set_shape([None, label_size])
    images_in = tf.comapt.v1.cast(images_in, dtype)
    labels_in = tf.comapt.v1.cast(labels_in, dtype)
