...
def process_reals(x, labels, lod, mirror_augment, drange_data, drange_net):
    with tf.compat.v1.keras.backend.name_scope('DynamicRange'):
        x = tf.compat.v1.cast(x, tf.float32)
        x = misc.adjust_dynamic_range(x, drange_data, drange_net)
    if mirror_augment:
        with tf.compat.v1.keras.backend.name_scope('MirrorAugment'):
            x = tf.compat.v1.where(tf.compat.v1.random.uniform([tf.shape(input=x)[0]]) < 0.5, x, tf.compat.v1.reverse(x, [3]))
    with tf.compat.v1.keras.backend.name_scope('FadeLOD'): # Smooth crossfade between consecutive levels-of-detail.
        s = tf.shape(input=x)
        y = tf.compat.v1.reshape(x, [-1, s[1], s[2]//2, 2, s[3]//2, 2])
        y = tf.math.reduce_mean(input_tensor=y, axis=[3, 5], keepdims=True)
        y = tf.compat.v1.tile(y, [1, 1, 1, 2, 1, 2])
        y = tf.compat.v1.reshape(y, [-1, s[1], s[2], s[3]])
        x = tflib.lerp(x, y, lod - tf.math.floor(lod))
    with tf.compat.v1.keras.backend.name_scope('UpscaleLOD'): # Upscale to match the expected input/output size of the networks.
        s = tf.shape(input=x)
        factor = tf.compat.v1.cast(2 ** tf.math.floor(lod), tf.int32)
        x = tf.compat.v1.reshape(x, [-1, s[1], s[2], 1, s[3], 1])
        x = tf.comapt.v1.tile(x, [1, 1, 1, factor, 1, factor])
...
def training_loop(
    ...
    with tf.compat.v1.keras.backend.name_scope('Inputs'), tf.device('/cpu:0'):
        ...
        Gs_beta              = 0.5 ** tf.compat.v1.div(tf.compat.v1.cast(minibatch_size_in, tf.float32), G_smoothing_kimg * 1000.0) if G_smoothing_kimg > 0.0 else 0.0
    ...
    for gpu in range(num_gpus):
        with tf.compat.v1.keras.backend.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.compat.v1.keras.backend.name_scope('DataFetch'):
                sched = training_schedule(cur_nimg=int(resume_kimg*1000), training_set=training_set, **sched_args)
                reals_var = tf.Variable(name='reals', trainable=False, initial_value=tf.compat.v1.zeros([sched.minibatch_gpu] + training_set.shape))
                labels_var = tf.Variable(name='labels', trainable=False, initial_value=tf.compat.v1.zeros([sched.minibatch_gpu, training_set.label_size]))
                ...
                reals_write = tf.compat.v1.concat([reals_write, reals_var[minibatch_gpu_in:]], axis=0)
                labels_write = tf.compat.v1.concat([labels_write, labels_var[minibatch_gpu_in:]], axis=0)
            ...
            with tf.compat.v1.control_dependencies(lod_assign_ops):
                with tf.compat.v1.keras.backend.name_scope('G_loss'):
                    G_loss, G_reg = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set, minibatch_size=minibatch_gpu_in, **G_loss_args)
                with tf.compat.v1.keras.backend.name_scope('D_loss'):
            ...
            if not lazy_regularization:
                ...
                if G_reg is not None: G_reg_opt.register_gradients(tf.math.reduce_mean(input_tensor=G_reg * G_reg_interval), G_gpu.trainables)
                if D_reg is not None: D_reg_opt.register_gradients(tf.math.reduce_mean(input_tensor=D_reg * D_reg_interval), D_gpu.trainables)
            G_opt.register_gradients(tf.math.reduce_mean(input_tensor=G_loss), G_gpu.trainables)
            D_opt.register_gradients(tf.math.reduce_mean(input_tensor=D_loss), D_gpu.trainables)

    # Setup training ops.
    data_fetch_op = tf.compat.v1.group(*data_fetch_ops)
    ...
    with tf.device('/gpu:0'):
        try:
            peak_gpu_mem_op = torch.cuda.max_memory_allocated(0)
        except tf.compat.v1.errors.NotFoundError:
