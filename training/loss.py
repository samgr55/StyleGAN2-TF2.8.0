...
def D_logistic_r1(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    with tf.compat.v1.name_scope('GradientPenalty'):
        real_grads = tf.gradients(ys=tf.reduce_sum(input_tensor=real_scores_out), xs=[reals])[0]
        gradient_penalty = tf.reduce_sum(input_tensor=tf.square(real_grads), axis=[1,2,3])
...
def D_logistic_r2(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    with tf.compat.v1.name_scope('GradientPenalty'):
        fake_grads = tf.gradients(ys=tf.reduce_sum(input_tensor=fake_scores_out), xs=[fake_images_out])[0]
        gradient_penalty = tf.reduce_sum(input_tensor=tf.square(fake_grads), axis=[1,2,3])
        gradient_penalty = autosummary('Loss/gradient_penalty', gradient_penalty)
        reg = gradient_penalty * (gamma * 0.5)
    return loss, reg
...
def D_wgan_gp(G, D, opt, training_set, minibatch_size, reals, labels, wgan_lambda=10.0, wgan_epsilon=0.001, wgan_target=1.0):
    ...
    with tf.compat.v1.name_scope('GradientPenalty'):
        ...
        mixed_grads = tf.gradients(ys=tf.reduce_sum(input_tensor=mixed_scores_out), xs=[mixed_images_out])[0]
        mixed_norms = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(mixed_grads), axis=[1,2,3]))
...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    ...
    with tf.compat.v1.name_scope('PathReg'):
        ...
        # Compute |J*y|.
        pl_noise = tf.random.normal(tf.shape(input=fake_images_out)) / np.sqrt(np.prod(G.output_shape[2:]))
        pl_grads = tf.gradients(ys=tf.reduce_sum(input_tensor=fake_images_out * pl_noise), xs=[fake_dlatents_out])[0]
