...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with tf.compat.v1.keras.backend.name_scope(None):
    ...
    def run(self,
        ...
        if key not in self._run_cache:
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.compat.v1.control_dependencies(None):
                ...
                with tf.device("/cpu:0"):
                    out_expr = [tf.compat.v1.concat(outputs, axis=0) for outputs in zip(*out_split)]
    ...
    def setup_weight_histograms(self, title: str = None) -> None:
        ...
        with tf.compat.v1.keras.backend.name_scope(name, default_name=None, values=None), tf.device(None), tf.compat.v1.control_dependencies(None):
...
def _legacy_output_transform_func(*expr, out_mul=1.0, out_add=0.0, out_shrink=1, out_dtype=None):
    ...
    if out_dtype is not None:
        if tf.dtypes.as_dtype(out_dtype).is_integer:
            expr = [tf.math.round(x) for x in expr]
        expr = [tf.dtypes.saturate_cast(x, out_dtype) for x in expr]
    return expr
